...
SELECT DISTINCT restStaff.first_name, restStaff.surname
FROM restStaff
JOIN restRest_table ON restRest_table.table_no IN (
    SELECT restBill.table_no FROM restBill WHERE restBill.bill_date = 160312
)
JOIN restRoom_management ON restRoom_management.room_name = restRest_table.room_name 
    AND restRoom_management.room_date = 160312
WHERE restRest_table.room_name = 'Blue' 
  AND (restStaff.staff_no = restRoom_manage
