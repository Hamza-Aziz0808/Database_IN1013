...
GROUP BY restBill.bill_date, restBill.waiter_no
...
SELECT 
    restRest_table.room_name, 
    COUNT(restBill.bill_no) AS total_bills
FROM restBill
JOIN restRest_table ON restBill.table_no = restRest_table.table_no
GROUP BY restRest_table.room_name;

SELECT 
    headwaiters.first_name, 
    headwaiters.surname, 
    SUM(restBill.bill_total) AS total_bill_amount
FROM restStaff AS waiters
JOIN restStaff AS headwaiters ON waiters.headwaiter = headwaiters.staff_no
JOIN restBill ON waiters.staff_no = restBill.waiter_no
GROUP BY headwaiters.staff_no
ORDER BY total_bill_amount DESC;

SELECT 
    restBill.cust_name, 
    AVG(restBill.bill_total) AS average_spent
FROM restBill
